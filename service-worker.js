...
const CACHE_VERSION = "tra-smart-hub-v1";
const STATIC_CACHE = `${CACHE_VERSION}-static`;
const RUNTIME_CACHE = `${CACHE_VERSION}-runtime`;

// Core "app shell" — always cached on install so the app opens instantly, even offline.
const APP_SHELL = [
  "/tra-revenue-hub/index.html",
  "/tra-revenue-hub/manifest.json",
  "/tra-revenue-hub/assets/css/style.css",
  "/tra-revenue-hub/assets/icons/icon-192.png",
  "/tra-revenue-hub/assets/icons/icon-512.png",
  "/tra-revenue-hub/assets/icons/icon-maskable-192.png",
  "/tra-revenue-hub/assets/icons/icon-maskable-512.png",
  "/tra-revenue-hub/assets/icons/apple-touch-icon.png",
  "/tra-revenue-hub/offline.html",
];
...
self.addEventListener("fetch", (event) => {
  ...
  if (request.mode === "navigate") {
    event.respondWith(
      fetch(request)
        ...
        .catch(() =>
          caches.match(request).then((cached) => cached || caches.match("/tra-revenue-hub/offline.html"))
        )
    );
    return;
  }
  ...
  const isStaticAsset =
    url.pathname.startsWith("/tra-revenue-hub/assets/") ||
